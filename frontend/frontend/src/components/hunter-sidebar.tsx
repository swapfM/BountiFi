...
interface HunterSidebarProps {
  collapsed: boolean;
  onToggle: () => void;
}

const menuItems = [
  { icon: Target, label: "Bounties", href: "/hunter/dashboard" },

  {
    icon: BadgeDollarSign,
    label: "Transactions",
    href: "/hunter/transactions",
  },
  { icon: Gem, label: "Rewards", href: "/hunter/rewards" },
];

export function HunterSidebar({ collapsed, onToggle }: HunterSidebarProps) {
    ...
    <>
              ...
              <Link key={item.href} href={item.href}>
                <div
                  className={cn(
                    ...
                    isActive
                      ? "bg-neon-green/20 text-neon-green border border-neon-green/30 neon-glow"
      ...
      {collapsed && (
        <Button
          ...
          size="icon"
          onClick={onToggle}
          className="fixed left-4 top-4 z-30 h-8 w-8 bg-card/80 backdrop-blur-sm border border-border hover:bg-neon-green/10"
        >
          <ChevronRight className="h-4 w-4" />
        </Button>
      )}
